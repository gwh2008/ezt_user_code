...
public class TxtSwitchButton extends CompoundButton {
	...
	public TxtSwitchButton(Context context) {
		this(context, null);
		mContext = context;
	}

	public TxtSwitchButton(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.switchStyle);
		mContext = context;
	}

	public TxtSwitchButton(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.TxtSwitchButton, defStyle, 0);// 从配置文件中获取相关配置
		mThumbDrawable = a.getDrawable(R.styleable.TxtSwitchButton_thumb);// 获取配置的轨迹资源
		mTrackDrawable = a.getDrawable(R.styleable.TxtSwitchButton_ezt_track);// 获取配置的开关资源
		mTextOn = a.getText(R.styleable.TxtSwitchButton_textOn);// 获取配置中
																// 选中情况下的显示文字 on
		mTextOff = a.getText(R.styleable.TxtSwitchButton_textOff);// 获取配置的
																	// 为选中情况下的显示文字
																	// off
		mThumbTextPadding = a.getDimensionPixelSize(
				R.styleable.TxtSwitchButton_ezt_thumbTextPadding, 0);// 获取配置的按钮字体大小
		mSwitchMinWidth = a.getDimensionPixelSize(
				R.styleable.TxtSwitchButton_ezt_switchMinWidth, 0);// 获取配置的最小宽度
		mSwitchPadding = a.getDimensionPixelSize(
				R.styleable.TxtSwitchButton_ezt_switchPadding, 0);// 获取配置的padding属性

		int appearance = a.getResourceId(
				R.styleable.TxtSwitchButton_ezt_switchTextAppearance, 0);// 获取配置文件中获取显示字体格式
		// 判断设定格式是否存在，存在的话就赋值
		if (appearance != 0) {
			setSwitchTextAppearance(attrs, context, appearance);
		}
		a.recycle();// 回收配置文件资源
	 ...
	 * 设置on,off显示的字体大小及格式
	 * */
	public void setSwitchTextAppearance(AttributeSet attrs, Context context,
			int resid) {
		mContext = context;
		TypedArray appearance = context.obtainStyledAttributes(attrs,
				R.styleable.TextAppearance, resid, 0);
