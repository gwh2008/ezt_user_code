 ...
 * @created 2013年03月03日
 */
public class DoctorList30Activity extends FinalActivity implements
    ...
    // private final String HOS_DOC_WB_LIST_DATA = "Hos_DocWBListData";//
    // 缓存key-医院找
    // private final String DEPT_DOC_WB_LIST_DATA = "Dept_DocWBListData";//
    // 缓存key-科室找
    ...
    private View noopsyche_evaluate_Layout;

    private View noopsyche_appoint_Layout;

    private View noopsyche_rate_Layout;

    private View noopsyche_level_Layout;

    private View noopsyche_threehos_layout;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_doctorlist30);
        ViewUtils.inject(DoctorList30Activity.this);
        ...
        switch (fromType) {
            ...
            case 2:// 科室找
                ...
                layout_tv1.setVisibility(View.VISIBLE);
                deptId = getIntent().getStringExtra("deptId");
        ...
        adapter = new DoctorListAdapter30(this, false);
    ...
    private TextView cbNumTv;// 是否有号

    private TextView cbThreeHosTv;// 是否三家医院
    // private CheckBox cbThreeHos;// 是否三家医院

    private TextView cbRateTv;// 预约率高到低
    // private CheckBox cbRate;// 预约率高到低

    private TextView cbLevelTv;// 医生级别
    // private CheckBox cbLevel;// 医生级别

    private TextView cbEvaluateTv;// 评价高低
    ...
    private void initialThreeBtPop() {
        ...
        cbNumTv = (TextView) noopsycheView
                .findViewById(R.id.noopsyche_appoint_Tv);
        cbRateTv = (TextView) noopsycheView
                .findViewById(R.id.noopsyche_rate_Tv);
        // cbRate = (CheckBox)
        // noopsycheView.findViewById(R.id.noopsyche_rate_cb);
        cbLevelTv = (TextView) noopsycheView
                .findViewById(R.id.noopsyche_level_Tv);
        // cbLevel = (CheckBox) noopsycheView
        // .findViewById(R.id.noopsyche_level_cb);

        cbEvaluateTv = (TextView) noopsycheView
                .findViewById(R.id.noopsyche_evaluate_Tv);
        // cbEvaluate = (CheckBox) noopsycheView
        // .findViewById(R.id.noopsyche_evaluate_cb);

        cbThreeHosTv = (TextView) noopsycheView
                .findViewById(R.id.noopsyche_threehos_Tv);
        ...
        noopsyche_evaluate_Layout = (View) noopsycheView
                .findViewById(R.id.noopsyche_evaluate_Layout);
        noopsyche_appoint_Layout = (View) noopsycheView
                .findViewById(R.id.noopsyche_appoint_Layout);
        noopsyche_rate_Layout = (View) noopsycheView
                .findViewById(R.id.noopsyche_rate_Layout);

        noopsyche_level_Layout = (View) noopsycheView
                .findViewById(R.id.noopsyche_level_Layout);

        noopsyche_threehos_layout = (View) noopsycheView
                .findViewById(R.id.noopsyche_threehos_layout);

        noopsyche_evaluate_Layout.setOnClickListener(this);
        noopsyche_appoint_Layout.setOnClickListener(this);
        noopsyche_rate_Layout.setOnClickListener(this);
        noopsyche_level_Layout.setOnClickListener(this);
        noopsyche_threehos_layout.setOnClickListener(this);
    ...
    private void initialChoicePopwindow(final View view, final int flag) {
            ...
            public void onDismiss() {
                switch (flag) {
                    case 1: {
                        ...
                        tvChoice1.setTextColor(getResources().getColor(
                                R.color.dark_black));
                    ...
                    case 2: {
                        ...
                        tvChoice2.setTextColor(getResources().getColor(
                                R.color.dark_black));
                    ...
                    case 3: {
                        ...
                        tvChoice3.setTextColor(getResources().getColor(
                                R.color.dark_black));
    ...
    public void docs_choice_tv1Click(View v) {
        if (BaseApplication.getInstance().isNetConnected) {
            ...
            if (choicePop != null) {
                if (!choicePop.isShowing()) {
                    choicePop.showAsDropDown(v, 0, 0);
                    // downBtn_tv1.startAnimation(animToDown);

                    tvChoice1.setTextColor(getResources().getColor(
                            R.color.main_color));
                    downBtn_tv1.setImageResource(R.drawable.green_arrowup);
                } else {
                    choicePop.dismiss();
                    // downBtn_tv1.startAnimation(animToUp);
                }
        ...
        if (BaseApplication.getInstance().isNetConnected) {
            switch (v.getId()) {
                case R.id.pop_bt:// 智能筛选确定
                    ...
                    isNum = cbNumTv.getText().toString().equals("否") ? false : true;// 是否有号
                    ...
                    isThreeHos = cbThreeHosTv.getText().toString().equals("否") ? false
                            : true;// 是否三家医院
                    isRate = cbRateTv.getText().toString().equals("否") ? false
                            : true;// 预约率高到低
                    isLevel = cbLevelTv.getText().toString().equals("否") ? false
                            : true;// 医生级别高低
                    isEvaluate = cbEvaluateTv.getText().toString().equals("否") ? false
                            : true;// 评价高低

                    choicePop.dismiss();
                    getData();
                    break;
                case R.id.noopsyche_evaluate_Layout: {
                    if (cbEvaluateTv.getText().toString().equals("否")) {
                        cbEvaluateTv.setText("是");
                    } else
                        cbEvaluateTv.setText("否");
                }
                break;
                case R.id.noopsyche_appoint_Layout: {
                    if (cbNumTv.getText().toString().equals("否")) {
                        cbNumTv.setText("是");
                    } else
                        cbNumTv.setText("否");
                }
                break;
                case R.id.noopsyche_rate_Layout: {
                    if (cbRateTv.getText().toString().equals("否")) {
                        cbRateTv.setText("是");
                    } else
                        cbRateTv.setText("否");
                }
                break;
                case R.id.noopsyche_level_Layout: {
                    if (cbLevelTv.getText().toString().equals("否")) {
                        cbLevelTv.setText("是");
                    } else
                        cbLevelTv.setText("否");
                }
                break;
                case R.id.noopsyche_threehos_layout: {
                    if (cbThreeHosTv.getText().toString().equals("否")) {
                        cbThreeHosTv.setText("是");
                    } else
                        cbThreeHosTv.setText("否");
                }
                break;
            }
...
//					Collections.sort(deptList, new PinyinComparatorDept());
    ...
    public void docs_lvItemClick(AdapterView<?> parent, View view,
        ...
        if (BaseApplication.getInstance().isNetConnected) {
            ...
            // 医生列表
            position = position - 1;
            if (position > -1&&position<adapter.getList().size()) {
                if (null != adapter.getList() && adapter.getList().size() > 0) {
                    ...
                    if (!isChoice) {
                        ...
                        Intent intent = new Intent(DoctorList30Activity.this,
        ...
        switch (parent.getId()) {
            ...
            case R.id.choice_hos_lt:// 分类小科室/医院科室
                ...
                if (fromType == 1) {// 医院找
                    ...
                    if (!isAllSearch) {
                        SystemPreferences.save(EZTConfig.KEY_DEPT_ID, this.deptId);
