...
public class BitmapUtils implements TaskHandler {
	...
	private String encodeUrl;// 将中文图片名字转码
    /**
	 * 中文转码
	 * 
	 * @param photo 2015-12-23 借鉴final框架的中文转码
	 * @return
	 */
	private String encodePhoto(String photo) {
		if (photo == null) {
			return "";
		}
		if (StringUtil.haveChineseStr(photo)) {
			encodeUrl = photo.substring(photo.lastIndexOf("/") + 1,
					photo.length());
			try {
				return photo.substring(0, photo.lastIndexOf("/") + 1)
